    ...
    foreach ($t_group_by_bug as $t_bug_id => $t_group) {
        if (!empty($t_group) && !is_empty_group($t_group)) {
            ...
            echo '<div align="center">',
                '<table cellspacing="0" class="width75"><tbody>',
                '<tr><td class="news-heading-public" style="background-color: '.$t_status_color.'"><span class="bold">'. $t_summary.'</span> - <span class="italic-small">' . $t_date_submitted . '</span></td></tr>';
            echo '<tr><td class="news-body">';
            foreach ($t_group as $t_bugnote) {
                ...
                if (!empty($t_note)) {
                    ...
                    '<div><span class="activity-date">',$t_date_submitted, '</span> <span class="bold">', $t_user_link , '</span> (', $t_bugnote_link, ')</div>',
                    '<div class="activity-note">', $t_note, '</div>',
                    '</div>';
                }
            }
            echo '</td></tr>';
